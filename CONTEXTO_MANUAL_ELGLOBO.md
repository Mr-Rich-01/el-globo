...
**Utilizadores do seed** (senha de todos: `[PASSWORD_POR_OMISSAO]` — ver `prisma/seed.ts`; podem ter sido alterados/removidos no go-live pelo script de wipe):

| Email | Role | Canal |
|---|---|---|
| [EMAIL_DO_SEED_ADMIN] | ADMIN | — (global) |
| [EMAIL_DO_SEED_GERENTE_RESTAURANTE] | GERENTE | RESTAURANTE |
| [EMAIL_DO_SEED_GERENTE_LOJA] | GERENTE | BOTTLESTORE |
| [EMAIL_DO_SEED_EMPREGADO_MESA] | EMPREGADO_MESA | RESTAURANTE |
| [EMAIL_DO_SEED_OPERADOR_BALCAO] | OPERADOR_BALCAO | RESTAURANTE |
| [EMAIL_DO_SEED_OPERADOR_BOTTLESTORE] | OPERADOR_BOTTLESTORE | BOTTLESTORE |
| [EMAIL_DO_SEED_COZINHEIRO] | COZINHEIRO | RESTAURANTE |
| [EMAIL_DO_SEED_GESTOR_STOCK] | GESTOR_STOCK | RESTAURANTE |
...
| `GESTOR_STOCK` | Gestor de inventário puro, **por canal** (como o GERENTE): só vê e gere o stock do canal que lhe for atribuído (canal RESTAURANTE inclui PISCINA). Acesso **exclusivo** à área de Stock (produtos, categorias, fichas técnicas, quebras, stock baixo). Sem acesso a vendas, caixas, mesas, abas ou dashboards financeiros — o middleware redireciona qualquer outra página para `/stock/produtos` e devolve 403 nas APIs fora do inventário. | Sim | `/stock/produtos` |

**Regra de canais** (`lib/canais.ts` — `canaisPermitidos`):
- Só o `ADMIN` vê os 3 canais (`RESTAURANTE`, `BOTTLESTORE`, `PISCINA`).
...
**Nota:** o `GESTOR_STOCK` **pode ser criado diretamente pela UI** de Utilizadores (`/backoffice/utilizadores`) — a função "Gestor de Stock" aparece no dropdown e o campo **Canal é obrigatório**, como para qualquer perfil não-admin. Um GESTOR_STOCK antigo sem canal atribuído fica sem acesso a dados até o ADMIN lhe definir um canal.
...
| Entrada de stock, desmanche, transferência, quebras | ADMIN, GERENTE, GESTOR_STOCK (GERENTE e GESTOR_STOCK só nos seus canais) |
...
| Stock | Stock Baixo | ⚠️ | `/stock/stock-baixo` | ADMIN, GERENTE, GESTOR_STOCK |
...
✅ **Resolvido:** o botão "+ Adicionar Consumo" no detalhe da aba redireciona agora corretamente para **`/piscina/comanda/[abaId]`** — comanda dedicada da aba (adaptada da comanda de mesa) que envia `canal: PISCINA` + `abaId` ao `POST /api/pedidos` e inclui o fecho de conta. A disponibilidade mostrada usa a linha de stock efetiva (PISCINA se ativa, senão fallback RESTAURANTE — espelha o desconto real do backend).

| URL | Propósito | Acesso |
|---|---|---|
| `/piscina/comanda/[abaId]` | Comanda da aba da piscina: catálogo, carrinho, envio para Cozinha/Bar e fecho de conta | ADMIN, GERENTE, EMPREGADO_MESA (canal PISCINA) |
...
2. Consumos são lançados na comanda da aba: botão "+ Adicionar Consumo" no detalhe da aba → **`/piscina/comanda/[abaId]`** (catálogo + carrinho + envio para Cozinha/Bar, como a comanda de mesa). A piscina sem stock próprio consome automaticamente do stock do **Restaurante** (fallback no backend).
...
- **Manual:** `/stock/produtos`, ação "Desmanchar" na linha do produto-caixa (ADMIN, GERENTE, GESTOR_STOCK; GERENTE e GESTOR_STOCK só nos seus canais). API `POST /api/stock/desmanchar`.
...
**Quem:** ADMIN (todos os canais), GERENTE e GESTOR_STOCK (apenas os seus canais). OPERADOR_BOTTLESTORE só consulta.
